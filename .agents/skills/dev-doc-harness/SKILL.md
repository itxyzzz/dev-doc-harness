...
| Draft or review small/medium specs and plans | `module:lifecycle`, `module:quality`, small/medium templates in `assets/templates/` | `module:models` when model or sub-agent strategy is assessed | Work item folder, short artifact ID, documentation matrix, validation commands, and draft review state are recorded. |
