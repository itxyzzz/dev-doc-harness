...
| Freeze planning packages | `references/planning-freeze-gates.md` (`module:freeze-gate`), `module:lifecycle` | Current work-item artifacts | Use `rule:freeze.draft-review`, `rule:freeze.approval-freeze`, `rule:freeze.stop-before-implementation`, `rule:lifecycle.commit-message-format`, `rule:lifecycle.changelog-before-commit`, and `rule:lifecycle.immutable-snapshots`; stage changelog source fragments for ordinary plan-only freezes. |
