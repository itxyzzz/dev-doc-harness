...
| Execute approved work and record variance | Approved artifacts, architecture snapshot when present, `module:lifecycle`, `module:execution-quality` | Phase validation commands and relevant project docs or tests | Use `rule:execution-quality.execution-thread-start` for a fresh task or model-transition rehydration, stay in approved scope, update matching changelog source fragments before commits, use planned commit subjects, and use `rule:lifecycle.variance-policy` for drift. |
