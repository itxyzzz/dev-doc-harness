...
| Execute approved work and record variance | Approved artifacts, architecture snapshot when present, `module:lifecycle`, `module:execution-quality` | Plan Checks and relevant project docs or tests | Use `rule:execution-quality.execution-thread-start`, then complete planned safe work, update the matching changelog fragment, and use `rule:lifecycle.variance-policy` for noteworthy drift. |
