...
| Freeze planning packages | `references/planning-freeze-gates.md` (`module:freeze-gate`), `module:lifecycle` | Current work-item artifacts | Use `rule:freeze.draft-review`, `rule:freeze.approval-freeze`, `rule:freeze.stop-before-implementation`, `rule:lifecycle.planning-shape`, `rule:lifecycle.commit-message-format`, and `rule:lifecycle.immutable-snapshots`; identify the frozen package, transition owner, next lifecycle stage, and upcoming-stage sub-agent assessment before continuity routing. |
| Execute approved work and record variance | Approved artifacts, architecture snapshot when present, `module:lifecycle`, `module:models`, `module:execution-quality`, `references/implementation-changelog.md` (`module:implementation-changelog`) | Plan Checks and relevant project docs or tests | Apply the execution-method cascade and route-specific reviewer contract after fresh authorization: use the approved method without a second generic method question, or record a fresh explicit operator override. Then use `rule:execution-quality.execution-thread-start`, complete planned safe work, update the implementation changelog fragment, and use `rule:lifecycle.variance-policy` for noteworthy drift. |
