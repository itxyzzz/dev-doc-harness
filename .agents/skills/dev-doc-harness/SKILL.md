...
| Execute approved work and record variance | Approved artifacts, architecture snapshot when present, `module:lifecycle`, `module:execution-quality` | Phase validation commands and relevant project docs or tests | Stay in approved scope, update `CHANGELOG.md` before commits, use planned commit subjects, and use `rule:lifecycle.variance-policy` for drift, including architecture drift that needs amendment handling. |
