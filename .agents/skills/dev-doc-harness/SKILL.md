...
All routes: `rule:models.non-proactive-delegation` treats a no-proactive-spawn environment rule as governing dispatch only; it does not excuse an assessment or required operator-approval request. This does not cause the small planning route to load `module:models`, its detailed policy, or Model and Sub-agent Strategy notation.
...
4. For small or medium work, draft both `<spec-filename>` and `<plan-filename>` in the same turn.
...
- [ ] Each small or medium package contains both canonical files, `<spec-filename>` and `<plan-filename>`.
...
- [ ] On applicable routes, a no-proactive-spawn dispatch constraint does not silently skip the required `module:models` assessment or operator-approval request; sub-agent use, if any, follows the active repository policy notation, and `Sub-agents: None` records a stage-specific fit reason.
