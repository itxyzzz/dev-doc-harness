...
| Draft or review large anchor specs | `module:lifecycle`, `module:quality`, `module:models`, `assets/templates/large-phased-work-item-spec.md` | Prior approved amendments when present | Anchor spec preserves handoff decisions and phase decomposition under `rule:lifecycle.large-anchor-spec`; normal output is an anchor-spec-only draft review state unless combined planning was explicitly requested. |
| Draft or review phase plans | Approved spec, amendments, prior phase outputs, recorded model/context strategy, `module:quality`, `module:lifecycle`, `module:models`, `assets/templates/large-phased-work-item-phase-plan.md` | `module:architecture` when phase scope changes router or ownership behavior | Phase plan is post-anchor, fresh-thread executable under `rule:quality.phase-plan-fresh-thread`, and does not reinterpret frozen decisions. |
