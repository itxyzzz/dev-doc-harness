...
| `module:quality` | `references/durable-planning-quality.md` | Normative policy | Baseline plain-language and durable spec and phase-plan quality, Specification Commitments, Verification Criteria, Plan Checks, asymmetric plan coverage, evidence-to-conformance status, and handoff preservation. |
...
| `module:execution-quality` | `references/context-and-quality-gates.md` | Advisory guidance | Context loading, consumer-side execution startup after the authorized transition, task preflight, environment compensation, and increment quality gates; no planning-transition or model-selection semantics. |
