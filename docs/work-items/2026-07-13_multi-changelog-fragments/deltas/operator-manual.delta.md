...
Before an ordinary commit, update the matching fragment with a new entry and run the grammar-only lint gate. Do not require the root changelog to be complete at that point. Before preparing a release, run lint and the root-completeness check; stop for either grammar or completeness failures. If valid unreleased entries are missing, run the explicit write consolidation and review the root-changelog diff before renaming the release group.
